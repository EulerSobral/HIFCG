...
function DocentesPage() {
  ...
  const filtered = useMemo(
    () => docentes.filter((d) => `${d.nome} ${d.email} ${d.matricula} ${d.especialidade}`.toLowerCase().includes(q.toLowerCase())),
    [docentes, q],
  );
  ...
  return (
    <div>
      ...
      <Card className="p-4">
        ...
        <Table>
          <TableHeader><TableRow><TableHead>Nome</TableHead><TableHead>E-mail</TableHead><TableHead>Matrícula</TableHead><TableHead>Especialidade</TableHead><TableHead className="w-24 text-right">Ações</TableHead></TableRow></TableHeader>
          <TableBody>
            {filtered.map((d) => (
              <TableRow key={d.id}>
